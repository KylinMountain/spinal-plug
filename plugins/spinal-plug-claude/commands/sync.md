...
If the fetch fails because the server is not running, state that it must be started separately with `mind-palace serve "$HOME/.spinal-plug/spinal-plug-central.db" 8787`.
