...
Set `SPINAL_PLUG_SYNC_URL` to a managed or self-hosted Spinal Plug sync endpoint. The public client does not start or bundle a Control Plane service.
...
Claude Code's native Auto Memory extraction is asynchronous. Spinal Plug's SessionStart, prompt, and Stop hooks opportunistically publish completed native topic files; an unavailable sync endpoint never blocks Claude Code. Use `/spinal-plug:share` when an immediate upload is required.
