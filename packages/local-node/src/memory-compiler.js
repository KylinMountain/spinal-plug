...
function initialStatus(event, payload, autoPromoteThreshold) {
    if (event.eventType === "memory.candidate.created")
        return "candidate";
    if (event.eventType === "memory.promoted")
        return "active";
...
function disputeId(spaceId, key, memoryIds) {
    return `dsp_${createHash("sha256")
        .update(`${spaceId}:${key}:${[...memoryIds].sort().join(":")}`)
        .digest("hex")
        .slice(0, 24)}`;
}
function isMemoryPayload(payload) {
    const candidate = payload;
    return Boolean(candidate.memoryId
        && candidate.kind
        && candidate.title
        && candidate.statement);
}
...
export class MemoryCompiler {
    ...
    compile(spaceId, input) {
        ...
        for (const { sequence, event } of events) {
            if (!isMemoryPayload(event.payload))
                continue;
        ...
        const isDescendant = (eventId, ancestorId, seen = new Set()) => {
        ...
        for (const resolution of resolutions) {
            ...
            for (const resolvedId of resolution.resolvedIds) {
                if (resolvedId === winner.memoryId)
                    continue;
                const resolved = records.get(resolvedId);
                if (!resolved || resolved.status === "deleted")
                    continue;
        ...
        const byUpdatedAt = (left, right) => right.updatedAt.localeCompare(left.updatedAt) || left.memoryId.localeCompare(right.memoryId);
        return {
            spaceId,
...
//# sourceMappingURL=memory-compiler.js.map