...
const LABELLED_SECRET_PATTERN = new RegExp(String.raw `${SECRET_LABEL}\s*[:=：]\s*["']?${GATED_SECRET_VALUE}["']?`, "i");
...
const BEARER_TOKEN_PATTERN = new RegExp(String.raw `\bbearer\s+["']?${SECRET_VALUE}["']?`, "i");
const VERB_LABELLED_SECRET_PATTERN = new RegExp(String.raw `${SECRET_LABEL}\s*(?:is|为|是)\s*["']?${GATED_SECRET_VALUE}["']?`, "i");
const SPACED_LABELLED_SECRET_PATTERN = new RegExp(String.raw `${SECRET_LABEL}\s+["']?${GATED_SECRET_VALUE}["']?`, "i");
...
export function memoryContainsLikelySecret(memory) {
    return [
        memory.title,
        memory.statement,
        memory.why,
        memory.howToApply,
        ...memory.references
    ].filter((value) => Boolean(value)).some(containsLikelySecret);
}
/** Reject secret-shaped strings anywhere in a durable object before it is persisted or projected. */
export function valueContainsLikelySecret(value) {
    const seen = new Set();
    const inspect = (candidate) => {
        ...
        return Object.entries(candidate).some(([key, entry]) => {
            if (typeof entry === "string" && containsLikelySecret(`${key}: ${entry}`))
                return true;
