...
M1 的目标是让同一台机器上的 Claude Code 与 Codex 使用同一份独立项目记忆。数据首先写入 Mind Palace 本地 SQLite WAL。本文件保留 M1 的原始收敛边界；跨设备同步、Codex 停止阶段候选提取和受限原生记忆投影已在后续阶段实现。
...
- 不把 Claude Code 或 Codex 的原生记忆当作事实源；仅能维护受 Mind Palace 保留标识保护的投影。
- 不上传完整会话记录。
- 不在 Hook 关键路径中进行网络请求。
